 ...
 */
final class VRTRBT_Admin_Suggestions {
	...
	public static function instance() {
		...
		if (!isset(self::$instance))
			self::$instance = new VRTRBT_Admin_Suggestions;
	...
	*/
	private function __construct() {
		...
		$timestamp = (int) get_option( 'vrtrbt_dismissed_on' );

		if ( empty( $timestamp ) || ( time() - $timestamp ) > (90 * 86400) ) {

			// Check AJAX submit
			if (defined('DOING_AJAX') && DOING_AJAX) {
				add_action( 'wp_ajax_vrtrbt_dismiss', array(&$this, 'dismiss'));
...
<script type="text/javascript">jQuery(function($) {	$(document).on('click', '.vrtrbt-dismiss .notice-dismiss', function() { $.post(ajaxurl, {'action':'vrtrbt_dismiss','nonce':$(this).parent().attr('data-nonce')}); }); });</script>
	<?php }
	...
	public function dismiss() {
		if (!empty($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'vrtrbt-dismiss')) {
			update_option('vrtrbt_dismissed_on', time(), true);
		}
	}
	...
	public function admin_notices() {
		...
		$plugin_data = get_plugin_data( VRTRBT_FILE );

		?><div class="vrtrbt-dismiss notice notice-success is-dismissible" data-nonce="<?php echo wp_create_nonce('vrtrbt-dismiss'); ?>">
