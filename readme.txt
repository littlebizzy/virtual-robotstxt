...
= 1.0.1 =
* Tweaked menu link to use Robots.txt instead of Virtual Robots.txt
* Added links to blog post and "Search Engine Visibility" on the settings page
* Changed containing folder (plugin slug) from virtual-robots-txt to virtual-robotstxt

= 1.0 =
* Initial (private) release based on private experimentation.
...
So for a /robots.txt request, this is the WP sequence hooks (more or less):
...
- init
- wp
- parse_query <- detect context via WP is_robots method, show the form content and ends execution.